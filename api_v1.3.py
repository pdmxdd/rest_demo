...
def make_json_response(json, status_code):
...
def api_get_dogs_root():
    ...
    return make_json_response(json.dumps(available_routes), 200)
...
def api_get_dogs():
    ...
    dogs = Dog.query.all()
    for dog in dogs:
        dogs_dict.append({"id": dog.id, "name": dog.name, "age": dog.age})
    return make_json_response(json.dumps(dogs_dict), 200)
...
def api_get_dog_by_id(dog_id):
    ...
    if requested_dog is None:
        return make_json_response(json.dumps(error_dict), 404)
    else:
...
def api_post_new_dog():
    ...
    return make_json_response(json.dumps(error_dict), 400)
...
if __name__ == "__main__":
    # app.debug = True
    from backend.util.seed_db import drop_create
    drop_create()
    app.run(port=8080)
